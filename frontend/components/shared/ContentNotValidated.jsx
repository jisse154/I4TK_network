  ...
  useEffect(() => {
    if (isConfirmed) {
      toast({
        ...
        description: "New content validated!!",
  ...
  return (
    <>
      <div className="w-full md:w-3/4 px-4 mb-8">
        <a className="block mb-6 overflow-hidden rounded-md" href="#"></a>
        <div className="mb-4">
          {contentObj.tokenURIJson.properties.category.map((c, index) => {
            return (
              <a
                key={index}
                className="inline-block py-1 px-3 text-xs leading-5 text-green-500 hover:text-green-600 font-medium  bg-green-100 hover:bg-green-200 rounded-full shadow-sm mr-5 "
                href="#"
              >
                {c}
              </a>
        ...
        <a
          className="inline-block mb-1 text-3xl leading-tight text-coolGray-800 hover:text-coolGray-900 font-bold hover:underline"
          href="#"
        >
          {contentObj.tokenURIJson.properties.title}
        </a>
        <span className="text-xl ml-5"># {contentObj.tokenId}</span>
        <p className="mb-4 text-base font-style: italic md:text-lg text-coolGray-800 font-small">
          {contentObj.tokenURIJson.properties.authors}
        </p>
        <p className="mb-4 text-base md:text-lg text-Gray-400 font-medium">
          {contentObj.tokenURIJson.properties.description}
        </p>
        <p className="mb-2 text-coolGray-500 font-medium">
          Posted by {contentObj.postedBy.substring(0, 6)}....{contentObj.postedBy.slice(-4)} • Proposed date:{" "}
          {contentObj.proposedDate}
        </p>
        <div className="flex justify-between">
          ...
          <span className="content-center">validations : {Number(nbValidation)}/4</span>
          <Button onClick={valide} className=" w-1/4 text-white bg-gray-500 ">
            Validate
          </Button>
        </div>
      </div>
...
export default ContentNotValidated;
