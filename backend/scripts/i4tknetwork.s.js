...
async function main() {
  ...
  const contractdeployed = new hre.ethers.Contract(contract.target, abi, deployer);
  //const tx = await contractdeployed.registerMember(contract.target,1);

  const tx = await contractdeployed.grantRole(deployer.address,"0xa49807205ce4d355092ef5a8a18f56e8913cf4a201fbe287825b095693c21775");

  await tx.wait();

  const tx1 = await contractdeployed.registerMember(contract.target,1);
